...
bool set_entry(Table *table, ObjString *key, Value value) {
    ...
    if (is_new_key && IS_NULL(entry->value)) table->length++;
...
ObjString *find_string(
    Table *from, const char *chars, int length, uint32_t hash
) {
    ...
    while (true) {
        ...
        if (entry->key == NULL && IS_NULL(entry->value)) return NULL;
